...
PYTHONPATH=. python main_moco.py environment.slurm=True \
    logging.wandb_project="mujoco_adapt_rep" logging.name="moco-v3_rn50_adroit_adapt_try3" \
    model.arch=resnet50 environment.ngpu=8 environment.world_size=1 \
    data.type="picklepaths" data.frameskip=2 \
    data.train_filelist="/checkpoint/yixinlin/eaif/datasets/adroit-expert-v0.1/" \
